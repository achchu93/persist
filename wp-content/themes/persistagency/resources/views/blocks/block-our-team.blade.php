<div class="{{ $class }} block-our-team scroll-fade-in anim-in fade-in"  style="--index: 0;">
  <div class="container mx-auto pb-24 pt-20 px-4 xl:pb-32">

    @if (!empty($header))
      <h2 class="bg-fill font-brushup inline-block leading-none pb-2 px-6 text-4xl text-white uppercase fade-in-element" style="--index: 1; background-image:url('@asset('images/brush-red.png')')">
        {{ $header }}
      </h2>
    @endif

    @if (!empty($content))
      <div class="leading-loose max-w-4xl mt-2 mt-6 mx-auto prose text-xl xl:mt-10 fade-in-element" style="--index: 2;">{!! $content !!}</div>
    @endif
    ...
    @if (!empty($items))
      <div class="gap-4 grid grid-cols-2 justify-center lg:grid-cols-4 max-w-5xl md:lg:grid-cols-3 mt-6 mx-auto xl:gap-x-10 xl:gap-y-20 xl:grid-cols-5 xl:mt-16">
        ...
        @foreach ($items as $key=>$item)
          <div class=" fade-in-element" style="--index: {{$key}};">
